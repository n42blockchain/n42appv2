...
import '../../widgets/chat/lottie_sticker_view.dart';
...
class MessageItem extends StatelessWidget {
  ...
  Widget _buildMessageContent(BuildContext context) {
    ...
    switch (message.type) {
      ...
      case MessageType.codeBlock:
        content = CodeBlockMessageWidget(
          code: message.content,
          language: message.metadata?.codeLanguage,
          fileName: message.metadata?.codeFileName,
          isDark: isDark,
        );
        break;
      case MessageType.tip:
        content = _buildTipMessage(isDark);
        break;
      default:
        content = _buildTextMessage(isDark, context);
    }
  ...
  Widget _buildTipMessage(bool isDark) {
    final metadata = message.metadata;
    final amount = metadata?.amount ?? '?';
    final token = metadata?.token ?? '';
    final isConfirmed = metadata?.transferStatus == 'confirmed';
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(
          colors: isDark
              ? [const Color(0xFF2D1A4E), const Color(0xFF1A1A2E)]
  ...
  Widget _buildStickerMessage() {
    final metadata = message.metadata;
    final httpUrl = metadata?.httpUrl ?? metadata?.mediaUrl ?? '';
    final kind = metadata?.stickerKind;
    final width = (metadata?.width ?? 160).clamp(96, 200).toDouble();

    Widget child;
    if (kind == 'lottie') {
      child = LottieStickerView(
        url: httpUrl,
        fallbackHttpUrl: metadata?.stickerFallbackHttpUrl,
        size: width,
        autoPlay: true,
        repeatCount: 1,
      );
    } else if (httpUrl.isEmpty) {
      child = const Icon(Icons.emoji_emotions_outlined, size: 64);
    } else {
      child = SizedBox(
        width: width,
        height: width,
  ...
  Widget _buildImageMessage(BuildContext context) {
    ...
    // 查看一次（view-once）：对接收端而言，消费后图片不可再看；
    // 对发送端保持正常显示（方便回看发过的内容）。
    final isViewOnceForPeer = message.viewOnce && !message.isFromMe;
    final isConsumed = message.viewOnceConsumed;
    ...
    return ImageMessageWidget(
      ...
      onTap: () {
        if (isViewOnceForPeer && !isConsumed) {
          // 打开即消费：先乐观标记，再走 repository 撤回
          context
              .read<ChatBloc>()
              .add(MarkViewOnceConsumed(message.id));
        }
        onTap?.call();
      },
      isViewOnce: message.isSelfDestructing || message.viewOnce,
      isExpired: message.isExpired || (isViewOnceForPeer && isConsumed),
