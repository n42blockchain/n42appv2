...
class GroupBloc extends Bloc<GroupEvent, GroupState> {
  ...
  bool _shouldSubscribeToMemberJoinEvents(BotConfig? config) {
    ...
    return config.supportsWebhookEvent(BotAutomationEventType.memberJoined) ||
        config.supportsWebhookEvent(BotAutomationEventType.memberLeft) ||
        config.supportsWebhookEvent(BotAutomationEventType.welcomeMessageSent);
  }

  /// Dispatch webhook and handle reply if present
  Future<void> _dispatchWebhook({
    ...
    final result = await service.dispatch(config: config, payload: payload);

    // If webhook responded with a reply, send it as a bot notice
    if (result.success && result.reply != null && result.reply!.body.isNotEmpty) {
      try {
        await _groupRepository.sendBotNotice(payload.roomId, result.reply!.body);
        debugLog('GroupBloc: Sent bot reply to ${payload.roomId}');
      } catch (e) {
        debugLog('GroupBloc: Failed to send bot reply: $e');
      }
    }
  }
