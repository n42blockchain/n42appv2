...
extension ChatBlocSendHandlers on ChatBloc {
  ...
  Future<void> onSendStickerMessage(
    ...
    try {
      ...
      await _messageRepository.sendStickerMessage(
        ...
        size: event.size,
        kind: event.kind,
        fallbackHttpUrl: event.fallbackHttpUrl,
      );
  ...
  /// 发送打赏消息
  Future<void> onSendTipMessage(
    SendTipMessage event,
    Emitter<ChatState> emit,
  ) async {
    if (!_canSendUserMessage(emit, 'tip')) return;

    emit(state.copyWith(isSending: true, clearError: true));
    try {
      await _messageRepository.sendTipMessage(
        _currentRoomId!,
        tipContent: event.tipContent,
      );
      emit(_buildPostSendState());
    } catch (e) {
      emit(state.copyWith(isSending: false, error: 'Failed to send tip: $e'));
    }
  }

  /// 发送代码块消息
  Future<void> onSendCodeBlockMessage(
    SendCodeBlockMessage event,
    Emitter<ChatState> emit,
  ) async {
    if (!_canSendUserMessage(emit, 'code block')) return;

    emit(state.copyWith(isSending: true, clearError: true));
    try {
      await _messageRepository.sendCodeBlockMessage(
        _currentRoomId!,
        code: event.code,
      ...
      emit(state.copyWith(isSending: false, error: 'Failed to send code: $e'));
    }
  }
