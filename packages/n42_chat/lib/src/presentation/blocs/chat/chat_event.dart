...
/// 标记一条"查看一次"消息已被本端查看/消费
class MarkViewOnceConsumed extends ChatEvent {
  final String messageId;
  const MarkViewOnceConsumed(this.messageId);
...
/// 发送打赏消息
class SendTipMessage extends ChatEvent {
  final Map<String, dynamic> tipContent;

  const SendTipMessage({required this.tipContent});

  @override
  List<Object?> get props => [tipContent];
}

/// 发送代码块消息
class SendCodeBlockMessage extends ChatEvent {
  final String code;
  final String? language;
  final String? fileName;

  const SendCodeBlockMessage({
    required this.code,
    this.language,
    this.fileName,
  });
...
class SendStickerMessage extends ChatEvent {
  ...
  final String? mimeType;
  final int? size;
  final String? kind; // image/gif/webp/apng/lottie
