...
extension ChatBlocActionHandlers on ChatBloc {
  ...
  ///
  /// 流程：
  /// 1. 本地乐观更新，将 [viewOnceConsumed] 置为 true。
  /// 2. 调 repository 发送 relation 事件 + redact 原消息。
  /// 3. 失败回滚。
  Future<void> onMarkViewOnceConsumed(
    MarkViewOnceConsumed event,
    Emitter<ChatState> emit,
  ) async {
    if (_currentRoomId == null) return;

    final original = List<MessageEntity>.unmodifiable(state.messages);
    final updated = state.messages.map((m) {
      if (m.id == event.messageId) {
        return m.copyWith(viewOnceConsumed: true);
      }
      return m;
    }).toList();
    ...
    try {
      await _messageRepository.markViewOnceConsumed(
        _currentRoomId!,
        event.messageId,
      );
    } catch (e) {
      debugLog('ChatBloc: markViewOnceConsumed failed for ${event.messageId}: $e');
      emit(state.copyWith(messages: original));
    }
  }
