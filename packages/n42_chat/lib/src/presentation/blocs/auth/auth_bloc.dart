...
class AuthBloc extends Bloc<AuthEvent, AuthState> {
  ...
  Future<void> _onPasskeyLogin(
    ...
    emit(state.copyWith(status: AuthStatus.loading, errorMessage: null));
    try {
      final result = await _authRepository.loginWithPasskey(
        homeserver: event.homeserver,
        rpId: event.rpId,
      );
      if (result.success && result.user != null) {
        ...
        emit(
          state.copyWith(
            status: AuthStatus.error,
            errorMessage: result.errorMessage ?? 'Passkey login failed',
            errorType: result.errorType,
          ),
        );
      }
    } catch (e) {
      debugLog('AuthBloc: Passkey login failed - $e');
      emit(
        state.copyWith(
          status: AuthStatus.error,
          errorMessage: 'Passkey login failed: $e',
        ),
      );
    }
  }
