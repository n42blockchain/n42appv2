...
class CallManager {
  ...
  // 活动群通话房间快照（语音 / 视频）：供外部 UI 观察。
  final StreamController<VideoRoomEntity?> _activeRoomController =
      StreamController<VideoRoomEntity?>.broadcast();
  VideoRoomEntity? _activeRoom;

  /// 当前活动的群通话（null 表示无）。
  VideoRoomEntity? get currentActiveRoom => _activeRoom;

  /// 订阅活动群通话状态变化（会话列表 banner、群聊头部均可使用）。
  Stream<VideoRoomEntity?> get activeRoomStream =>
      _activeRoomController.stream;

  void _updateActiveRoom({
    required String conversationId,
    required String livekitRoomName,
    required String displayName,
    required bool hasVideo,
  }) {
    final entity = VideoRoomEntity(
      conversationId: conversationId,
      livekitRoomName: livekitRoomName,
      displayName: displayName,
      participantCount: _liveKitService?.participants.length ?? 1,
      hasVideo: hasVideo,
      isActive: true,
    );
    _activeRoom = entity;
    _activeRoomController.add(entity);
  }
  ...
  void _onLiveKitChanged() {
    final current = _activeRoom;
    if (current == null) return;
    final newCount = _liveKitService?.participants.length ?? current.participantCount;
    if (newCount == current.participantCount) return;
    final updated = current.copyWith(participantCount: newCount);
    _activeRoom = updated;
    _activeRoomController.add(updated);
  }
  ...
  Future<void> dispose() async {
    ...
    await _liveKitService?.leaveMeeting();
    _clearActiveRoom();
    await _activeRoomController.close();
