...
import '../protocols/passkey_bridge_protocol.dart';
...
abstract class IAuthRepository {
  ...
  /// 是否可用 Passkey 登录。宿主未注入 `PasskeyBridge` 或设备不支持时返回 false。
  Future<bool> isPasskeyAvailable();

  /// 使用 Passkey 登录。
  ///
  /// [homeserver] 目标 Matrix Homeserver。
  /// [rpId] WebAuthn Relying Party ID（通常为 homeserver 域名）。
  Future<AuthResult> loginWithPasskey({
    required String homeserver,
    required String rpId,
  });

  /// 注册一个新的 Passkey（用户需先以密码登录后在设置页绑定）。
  Future<bool> registerPasskey({required String label, required String rpId});

  /// 列出本设备已注册的 Passkey。
  Future<List<PasskeyCredentialSummary>> listPasskeys();
