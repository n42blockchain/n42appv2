...
class AuthRepositoryImpl implements IAuthRepository {
  ...
  Future<bool> isPasskeyAvailable() async {
    final bridge = N42Chat.passkeyBridge;
  ...
  Future<AuthResult> loginWithPasskey({
    ...
    if (bridge == null) {
      return AuthResult.failure(
        'Passkey bridge not configured',
        type: AuthErrorType.unknown,
      );
    }
    try {
      // Step 1: 向 homeserver 请求 challenge。此处走一个自定义 endpoint，
      // 若服务端尚未实现则降级到本地挑战（仅调试）——正式环境须接入
      // `/_matrix/client/v3/login/passkey/challenge` 类端点。
      final challenge = _randomChallenge();

      // Step 2: 通过桥执行 WebAuthn 断言。
      final assertion = await bridge.authenticate(
        challenge: challenge,
        rpId: rpId,
      );

      // Step 3: 提交断言给服务端换取 Matrix access_token。
      // 当前服务端未部署该接口时，此调用应由宿主扩展 `PasskeyBridge` 实现，
      // 或改用 Matrix 2.0 `m.login.sso` 流程。此处返回 failure 以便 UI 提示。
      return AuthResult.failure(
        'Passkey login requires server-side endpoint: credentialId=${assertion.credentialId}',
        type: AuthErrorType.unknown,
      );
    } catch (e) {
      return AuthResult.failure('Passkey login failed: $e',
          type: AuthErrorType.unknown);
    }
  }
  ...
  Future<bool> registerPasskey({
    ...
    try {
      ...
      debugLog('registerPasskey failed: $e');
      return false;
    }
  }

  @override
  Future<List<PasskeyCredentialSummary>> listPasskeys() async {
    final bridge = N42Chat.passkeyBridge;
  ...
  Future<void> removePasskey(String credentialId) async {
    ...
    await bridge.remove(credentialId);
  }
