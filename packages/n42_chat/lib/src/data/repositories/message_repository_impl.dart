...
class MessageRepositoryImpl implements IMessageRepository {
  ...
  Future<MessageEntity?> sendTipMessage(
    ...
    final eventId = await _messageDataSource.sendTipMessage(
      roomId,
  ...
  Future<MessageEntity?> sendCodeBlockMessage(
    ...
    final eventId = await _messageDataSource.sendCodeBlockMessage(
      roomId,
      code: code,
      language: language,
      fileName: fileName,
  ...
  @override
  Future<bool> markViewOnceConsumed(String roomId, String messageId) async {
    try {
      // 1) 发送 relation 事件告知发送方"已查看"
      await _messageDataSource.sendRoomEvent(
        roomId: roomId,
        type: 'org.n42.view_once_consumed',
        content: {
          'm.relates_to': {
            'rel_type': 'org.n42.view_once_consumed',
            'event_id': messageId,
          },
        },
      );
  ...
  MessageEntity _getCachedOrMapMessage(
    ...
    if (_messageEntityCache.length > 200) {
