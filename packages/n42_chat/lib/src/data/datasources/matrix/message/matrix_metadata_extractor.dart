...
class MatrixMetadataExtractor {
  ...
  MessageMetadata? extractMetadataWithHttpUrl(matrix.Event event) {
    ...
    // 贴纸消息（m.sticker）：识别 Lottie/动画贴纸
    if (event.type == matrix.EventTypes.Sticker) {
      final n42Sticker = event.content['org.n42.sticker'] as Map<String, dynamic>?;
      final mimeType = info?['mimetype'] as String?;
      final kindFromN42 = n42Sticker?['kind'] as String?;
      // 回退：根据 mimeType 推断
      String? kind = kindFromN42;
      if (kind == null && mimeType != null) {
        final lower = mimeType.toLowerCase();
        if (lower.contains('lottie') || lower == 'application/json') {
          kind = 'lottie';
        } else if (lower.contains('gif')) {
          kind = 'gif';
        } else if (lower.contains('webp')) {
          kind = 'webp';
        } else if (lower.contains('apng')) {
          kind = 'apng';
        } else {
          kind = 'image';
        }
      }
      final fallback = (n42Sticker?['fallback_http_url'] as String?) ??
          (event.content['org.n42.sticker.thumbnail'] as String?);
      return MessageMetadata(
        ...
        stickerKind: kind,
        stickerPackId: n42Sticker?['pack_id'] as String?,
        stickerId: n42Sticker?['sticker_id'] as String?,
        stickerFallbackHttpUrl: fallback,
      );
    }
    ...
    // 代码块信息
    if (event.content['msgtype'] == 'n42.code_block') {
      final codeData =
          event.content['org.n42.code'] as Map<String, dynamic>?;
      return MessageMetadata(
        codeLanguage: codeData?['language'] as String?,
        codeFileName: codeData?['file_name'] as String?,
      );
    }
