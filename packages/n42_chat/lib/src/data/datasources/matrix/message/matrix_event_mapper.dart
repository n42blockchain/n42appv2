...
class MatrixEventMapper {
  ...
  MessageEntity mapEventToMessage(matrix.Event event, matrix.Room room) {
    ...
    // 解析"查看一次"字段（org.n42.view_once）
    final viewOnceData =
        event.content['org.n42.view_once'] as Map<String, dynamic>?;
    final viewOnce = viewOnceData != null;
    // consumed 可由发送端设为 true（例如对方已查看后的关联事件反映到本消息元数据）；
    // 另一条路径是读取消息 redaction / relation 标志，详见 MessageRepository。
    final viewOnceConsumed = (viewOnceData?['consumed'] as bool?) ?? false;
