...
class MatrixMessageSender {
  ...
  ///
  /// [kind] 资源类型（image/gif/webp/apng/lottie），默认 image。
  /// [fallbackHttpUrl] Lottie 贴纸的静态回退缩略图，供不支持 Lottie 的客户端渲染。
  Future<String?> sendStickerMessage(
    ...
    try {
      ...
      final effectiveMime = mimeType ??
          (kind == 'lottie' ? 'application/lottie+json' : 'image/png');
      final info = <String, dynamic>{'mimetype': effectiveMime};
      if (width != null) info['w'] = width;
      if (height != null) info['h'] = height;
      if (size != null) info['size'] = size;

      final n42Sticker = <String, dynamic>{
        'pack_id': packId,
  ...
  /// 发送代码块消息
  ///
  /// 使用自定义 msgtype `n42.code_block`，body 为纯文本代码，
  /// `org.n42.code` 携带语言和文件名元数据。不支持此 msgtype 的客户端
  /// 会显示 body 原文，兼容性安全。
  Future<String?> sendCodeBlockMessage(
    String roomId, {
    ...
    final room = _client?.getRoomById(roomId);
    if (room == null) throw Exception('房间不存在: $roomId');

    final content = <String, dynamic>{
      'msgtype': 'n42.code_block',
      'body': code,
      'format': 'org.matrix.custom.html',
      'formatted_body': '<pre><code class="language-${language ?? ''}">'
          '${_escapeHtml(code)}</code></pre>',
      'org.n42.code': <String, dynamic>{
  ...
  Future<String?> sendTipMessage(
    ...
    final room = _client?.getRoomById(roomId);
    if (room == null) throw Exception('房间不存在: $roomId');
    return await room.sendEvent(tipContent);
  }
