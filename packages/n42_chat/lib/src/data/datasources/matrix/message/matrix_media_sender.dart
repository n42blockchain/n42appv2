...
class MatrixMediaSender {
  ...
  ///
  /// [viewOnce] 勾选"查看一次"，发送 `org.n42.view_once` 标记；
  /// 同时把 [selfDestructAfter] 强制为 0（兼容不支持该标记的客户端）。
  Future<String?> sendImageMessage(
    ...
    try {
      ...
      // 查看一次（View-once）：独立于 selfDestruct 的标记；
      // 同时写入 n42.self_destruct={after:0} 作为兼容降级：不支持
      // 该字段的客户端打开后会立即触发销毁管线。
      if (viewOnce) {
  ...
  Future<String?> sendVideoMessage(
    ...
    try {
      ...
      // 查看一次（View-once）
      if (viewOnce) {
        content['org.n42.view_once'] = {'consumed': false};
        content['n42.self_destruct'] ??= {'after': 0};
      }
