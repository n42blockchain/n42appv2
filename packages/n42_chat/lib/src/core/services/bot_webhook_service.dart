...
/// Webhook response reply — if present, the bot wants to send a message back
class BotWebhookReply {
  final String body;
  final String format; // 'text' or 'markdown'

  const BotWebhookReply({required this.body, this.format = 'text'});

  factory BotWebhookReply.fromJson(Map<String, dynamic> json) {
    return BotWebhookReply(
...
class BotWebhookDispatchResult {
  ...
  final bool skipped;
  final BotWebhookReply? reply;

  factory BotWebhookDispatchResult.sent(int statusCode, {BotWebhookReply? reply}) =>
      BotWebhookDispatchResult(success: true, statusCode: statusCode, reply: reply);
...
class BotWebhookService {
  ...
  Future<BotWebhookDispatchResult> dispatch({
    ...
    try {
      ...
      // Parse reply from response body if present
      BotWebhookReply? reply;
      try {
        final responseBody = jsonDecode(response.body) as Map<String, dynamic>;
        if (responseBody.containsKey('reply')) {
          reply = BotWebhookReply.fromJson(
              responseBody['reply'] as Map<String, dynamic>);
        }
      } catch (_) {
        // Response body is not JSON or has no reply — that's fine
      }

      return BotWebhookDispatchResult.sent(response.statusCode, reply: reply);
    } catch (e) {
