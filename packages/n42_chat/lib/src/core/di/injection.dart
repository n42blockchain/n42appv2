...
Future<void> _registerServices(N42ChatConfig config) async {
  ...
  // Lottie 贴纸缓存
  getIt.registerLazySingleton<LottieCacheService>(
    () => LottieCacheService(),
  );

  // 聊天锁（共享单例，避免每次 new）
  getIt.registerLazySingleton<ChatLockService>(() => ChatLockService());
  ...
  // 通话增强：AI 降噪 + 虚拟背景
  getIt.registerLazySingleton<AudioProcessingService>(
    () => AudioProcessingService(),
    dispose: (service) => service.dispose(),
  );
  getIt.registerLazySingleton<VideoProcessingService>(
