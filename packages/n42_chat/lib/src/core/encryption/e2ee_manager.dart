...
class E2EEManager {
  ...
  /// 缓存的恢复密钥（仅在当前会话中有效）
  /// 自动在 [_recoveryKeyTtl] 后清除，减少内存暴露窗口。
  String? _cachedRecoveryKey;
  Timer? _recoveryKeyExpiryTimer;
  static const _recoveryKeyTtl = Duration(minutes: 5);
  ...
  /// 也会被自动过期计时器调用（[_recoveryKeyTtl] 后自动清除）。
  void clearCachedRecoveryKey() {
    _cachedRecoveryKey = null;
    _recoveryKeyExpiryTimer?.cancel();
    _recoveryKeyExpiryTimer = null;
  }

  /// 设置恢复密钥并启动自动过期计时器
  void _setCachedRecoveryKey(String key) {
    _cachedRecoveryKey = key;
    _recoveryKeyExpiryTimer?.cancel();
    _recoveryKeyExpiryTimer = Timer(_recoveryKeyTtl, () {
      debugLog('E2EEManager: Recovery key auto-cleared after $_recoveryKeyTtl');
      clearCachedRecoveryKey();
    });
  }
  ...
  Future<String?> createRecoveryKey({String? passphrase}) async {
    ...
    try {
      ...
      // 3. 缓存恢复密钥（自动5分钟过期）
      _setCachedRecoveryKey(recoveryKey);
  ...
  Future<void> unlockWithPassphrase(String passphrase) async {
    ...
    try {
      ...
      // 缓存恢复密钥（自动5分钟过期）
      final rk = openSsss.recoveryKey;
      if (rk != null) _setCachedRecoveryKey(rk);
