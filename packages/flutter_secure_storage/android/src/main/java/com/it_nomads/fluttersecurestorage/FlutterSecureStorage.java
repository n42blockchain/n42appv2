...
public class FlutterSecureStorage {
    ...
    private void continueInitialize(FlutterSecureStorageConfig config, SecurePreferencesCallback<Void> callback) {
        ...
        if (!N42StorageMigration.isManaged(config)) LegacyNamespaceKeyRecovery.recoverIfNeeded(context, config);
    ...
    private void recoverBiometricNamespaceKeyIfNeeded(FlutterSecureStorageConfig config,
        ...
        if (N42StorageMigration.isManaged(config) || !BiometricNamespaceKeyRecovery.isRecoveryNeeded(context, config)) {
            callback.onSuccess(null);
            return;
        }
