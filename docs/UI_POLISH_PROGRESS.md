...
| **全局字阶（2-字段）** | 🟡 机械层完成 | 2026-06-07 | 全 features（532 处） | 仅 `fontSize+color` 两字段 TextStyle → `AppTypography.<role>.copyWith(color:)`（codemod，color 含逗号自动跳过）。**剩余 ~763 setSp** 带字重/多字段/响应式变量/图标尺寸，需逐站点手工（wallet 大头）。|
