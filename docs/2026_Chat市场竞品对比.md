...
> **已据此真实补齐（2026-06-26 本次落地）**：① **代码块 `n42.code_block`**——真实现 `MessageType.codeBlock` + `CodeBlockMessageWidget`（flutter_highlight 着色/行号/语言徽章/复制/围栏回退）；② **待办提醒**——`FavoriteEntity.dueAt/isCompleted` + `ReminderService`（存储/周期检查/本地通知）+ 消息「Remind」入口；③ **查看一次复核勘误**——经核 view-once **能力其实已在**（self-destruct-on-view），原判"凭空虚构"不准，降级为"符号名不实"（见下表第 4 行已改）。其余项需原生/后端/key（Gemma=MediaPipe 原生、MLS=Rust FFI、系统集成/虚拟背景=原生、通话 E2EE=WebRTC insertable、法币=MoonPay key、订阅=支付后端、实时字幕=STT+通话集成），非纯客户端，暂仍为缺失，待按真实工作量排期。

| 章 | 声称（正文） | 实况 | 证据 |
|---|---|---|---|
| 9 | **本地设备推理 Gemma 整套**（LocalLlmService/LocalLlmBridge/LocalAiDatasource/AiProviderRouter/LocalLlmSettingsPage/LocalLlmHandler.kt）—— P3"质变"核心卖点 | **不存在**。全仓 **0 个 .kt 文件**，AI 100% 云端 OpenAI 兼容 API | grep `LocalLlm/Gemma/AiProviderRouter` = 0 |
| 10 | **MLS RFC 9420**（MlsProtocol/MlsManager，"Olm↔MLS 双栈"）| **不存在**，连接口占位都没有 | grep `Mls/MLS/9420/OpenMls` = 0 |
| 12 | **系统级集成 SystemIntegrationService**（iOS Live Activities / Android Bubbles / Desktop 托盘）| Service 不存在，无任何平台通道（无 .kt/.swift、pubspec 无托盘类依赖）| grep = 0 |
| 3 | **虚拟背景/模糊 VirtualBackgroundProcessor**（MLKit SelfieSegmentation / Isolate）| 空壳：通话背景仅 `_updateBackgroundProcessing()` 记日志；无分割库（只有人脸检测）。唯一真实的 FaceBlurUtil 是**静态图片人脸打码**非实时视频 | grep `VirtualBackground/SelfieSegmentation` = 0 |
| 3 | **实时字幕 LiveCaptionService**（+STT 轮询 +Overlay）| 完全不存在，且无任何 STT 依赖（`flutter_tts` 是反方向的朗读）| grep = 0 |
| 3 | **通话端到端加密**（Matrix VoIP E2EE）| 无应用层 E2EE；1:1 仅 WebRTC DTLS-SRTP 传输层，群通话经 LiveKit SFU **明文可见** | grep `e2ee/sframe/frameCrypto` in webrtc = 0 |
| 7 | **法币出入金 FiatRampBridge/FiatRampService/FiatRampPage** | 两仓零实现，纯文档话术 | grep `FiatRamp/moonpay/transak` = 0（仅命中本文档）|
| 7 | **打赏 Tip `n42.tip` + 渐变气泡** | 无 msgtype、无组件 | grep `n42.tip/TipBubble` = 0 |
| 7 | **订阅 SubscriptionService/SubscriptionPlan** | 无付费订阅实体（41 处 Subscription 全是 Rx StreamSubscription）| grep 类 = 0 |
...
| 6 | **短视频 VideoFeedPage 沉浸式 PageView** | 完全不存在 | grep `VideoFeed/short_video/reels` = 0 |
