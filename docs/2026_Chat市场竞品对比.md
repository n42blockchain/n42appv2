...
> **已据此真实补齐（2026-06-26 本次落地）**：① **代码块 `n42.code_block`**（见下表已划改）——真实现 `MessageType.codeBlock` + `CodeBlockMessageWidget`（flutter_highlight 语法着色 + 行号 + 语言徽章 + 复制 + 围栏回退）。其余项需原生/后端/key（Gemma=MediaPipe 原生、MLS=Rust FFI、系统集成/虚拟背景=原生、通话 E2EE=WebRTC insertable、法币=MoonPay key、订阅=支付后端、实时字幕=STT+通话集成），非纯客户端，暂仍为缺失，待按真实工作量排期。
...
| 11 | **待办提醒 ReminderService + FavoriteEntity.dueAt** | 完全不存在 | grep = 0 |
...
| 4 | **查看一次 `org.n42.view_once` + markViewOnceConsumed** | 点名机制不存在（仅有相近但不同的"自毁消息"selfDestructAfter）| grep = 0 |
