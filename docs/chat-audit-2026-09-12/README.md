...
iPhone 13 Pro Max 与 Xiaomi 25098RA98C 各 **19 项真机用例通过**：15 项数据库契约、3 项新增 UI、1 项表情/菜单/RTL。双端正常应用已覆盖恢复并启动，小米首次安装时间未变，iPhone 容器校验文件一致。24 张夹具截图及逐项结果见 [真机报告](DEVICE_ACCEPTANCE_2026-09-13.md)。历史账户数据完整性仍不在本次认证范围内。

小米首轮暴露了 SQLCipher 加载和多用例截图状态问题。正式插件 `657a66bb1a5a5963f6460e5cc60b305fcc272de3` 修复了媒体数据库后台 isolate 的加载配置；新增生产初始化路径的写入、关闭、重开回归。设备测试脚本只连接已安装应用，避免 Flutter 覆盖安装失败后的卸载重试。宿主 36 项质量检查、4 项贴纸兼容检查和插件 64 项存储检查通过；静态检查 0 错误、0 警告、155 条 info。767 个运行时文件与实际 Git 依赖哈希一致。

## 分模块覆盖率扩展（2026-09-13）

正式插件提交：`58ed5f9b07277a211f84da505f6718c2716a8f86`。新增 **140 项行为测试**，最终全量 **6,007 项通过、1 项在线凭据测试跳过**。原始覆盖率 **19.23% → 21.21%**；排除生成代码的参考视图 **31.03% → 33.67%**，CI 门槛未改变。消息操作 98.91%、动态 97.44%、投票 99.52%、注册页 84.91%、重置密码页 91.05%。详见 [逐模块报告](COVERAGE_EXPANSION_2026-09-13.md) 与 [机器可读统计](COVERAGE_EXPANSION_2026-09-13.json)。

修复了会话恢复被自身认证锁拒绝、Matrix 错误分类、空白全文搜索、收藏失败后虚假成功与并发丢失、空转发结果误报成功、注册开关背景、投票测验多选和窄屏溢出、图片下载策略竞态及阅后即焚预览尺寸。真实 SQLite 在内存数据库中执行；Matrix 和支付边界采用受控替身，不能视为线上消息/资金验收。

宿主集成：实际 Git 依赖下 136 项回归通过，宿主相关 109 项检查通过；静态检查 0 错误、0 警告、155 条 info。767 个 lib/assets 文件与 Git 缓存哈希一致。此次未重跑宿主完整覆盖率，宿主整体数据仍使用明确标注的历史基线。

以下为 360×850 本地 Widget 截图，使用系统字体；未安装或改动真机应用。阿拉伯语截图中的 Schedule/Quiz 等旧文案仍属于 QA-006 的翻译待办。

| 英文投票 | 阿拉伯语投票 | 阅后即焚预览 |
|---|---|---|
| [截图](widget-coverage-poll-en.png) | [截图](widget-coverage-poll-ar.png) | [截图](widget-coverage-view-once.png) |

新增未决项：收藏跨键删除非事务（STORAGE-001）、标签/备注无 UI 读写入口（UI-001）、邮箱确认参数与 Matrix 验证链路不一致（AUTH-001）。正式插件 OPEN_ISSUES.md 保存证据和下一步，未把这些功能标成完成。
...
```sh
./scripts/run_chat_device_acceptance.sh <device-id> <vm-service-uri>
```
...
详见 [重试与恢复记录](DEVICE_RETRY_2026-09-12.md)。2026-09-13 起，该脚本只连接已启动的测试应用；先显式覆盖安装，再传入 VM service URI，测试后覆盖安装正常入口；iOS 签名从 Mac 的 GUI Terminal 执行。
...
- 独立 Settings 的账号回调与默认持久化已统一（INTEGRATION-002 已解决）；跨端真实账号验收仍见 QA-005。
- 533 个直接 UI 文案候选仍待逐模块审查（QA-006）；26 份 ARB 完整不能代替此项。
