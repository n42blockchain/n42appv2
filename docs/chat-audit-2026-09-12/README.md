...
## 分模块覆盖率第四轮（2026-09-14）

正式插件 `ce0483338d859e4c483506fe3a7f1c29e1a581ec` 新增 **105 项测试**：联系人 32、会话 24、持币校验 49。插件全量 **6,271 项通过、1 项跳过**；联系人仓库 **33.81% → 90.07%**、会话仓库 **53.89% → 79.64%**、群组仓库 **41.15% → 65.77%**。原始整体覆盖率 **21.66% → 21.82%**，非生成代码参考视图 **34.41% → 34.67%**。

宿主新 Git 依赖下 **303 项检查通过**，静态检查 0 错误、0 警告、155 条 info；767 个运行时文件哈希一致。修复备注/空格搜索、黑名单备注初始化、持币检查读取失败时默认放行、原生币精度丢失和未知链回退到 ETH。真实接受邀请与设置 Bloc 的失败路径已有回归。**GROUP-001 仍开放**：别名/直接 SDK 加入未接入同样的校验，服务端强制准入尚未验证。详见 [第四轮报告](COVERAGE_ROUND4_2026-09-14.md)，包含完整 trace、哈希和旧实现失败证据。本轮未更改真机安装与原验收 tag。

## 分模块覆盖率第三轮（2026-09-14）

正式插件 `dba781335dfe4238b5de1a5efbac709d62c28751` 新增 **74 项测试**：搜索仓库 43，真实 SQLite 下归档搜索 31。插件全量 **6,166 项通过、1 项跳过**。搜索仓库 **12.63% → 98.56%**，归档搜索服务 **0% → 100%**，归档数据库 **56.70% → 59.71%**；整体原始覆盖率 **21.44% → 21.66%**，非生成代码参考视图 **34.05% → 34.41%**。

宿主固定 Git 依赖下 **198 项检查通过**，静态检查 0 错误、0 警告、155 条 info；767 个运行时文件与 Git 依赖哈希一致。修复归档搜索忽略界面筛选条件，以及加载更多后选中消息发生偏移或索引失效。保留旧实现失败日志、最终全量 trace 和哈希，详见 [第三轮报告](COVERAGE_ROUND3_2026-09-14.md)。本轮没有新的真机验收，原验收 tag 保持原指向。

## 分模块覆盖率第二轮（2026-09-13）

正式插件 `e38d6fb43bd61227352ec9cdb54dbb18e8e7c069` 新增 **84 项行为测试**：贴纸 38、备份 34、导出 12。重新测量的全量基线 **6,008 → 6,092 项通过**，1 项在线凭据测试跳过。贴纸仓库 **33.10% → 93.10%**，备份服务 **43.42% → 66.72%**，导出服务 **42.77% → 83.83%**；插件整体原始覆盖率 **21.22% → 21.44%**，排除生成代码的参考视图 **33.69% → 34.05%**。

修复连续备份覆盖、密码备份 GCM 输出缓冲区导致的认证失败、同秒同名导出覆盖。旧 v3 补零格式只在认证成功时恢复，独立 Python 合成样本验证互操作。新增用例接入宿主测试入口，实际 Git 依赖下 **124 项检查通过**；宿主静态检查 0 错误、0 警告、155 条 info，插件 0 错误、0 警告、173 条 info。详见 [第二轮报告](COVERAGE_ROUND2_2026-09-13.md)，内含前后完整 lcov 压缩文件和哈希。此次未变更真机安装和上轮验收 tag。
