> 历史快照：2026-09-12 前的报告。含未复核完成标记、旧依赖和旧市场判断，不作为当前验收依据。
...
| 28 | 空间音频 | ❌ |
| 29 | PSTN 互通 | ❌（需电信网关） |
| 30 | ~~白板协作 / 手写 Digital Touch~~ | ✅ **单人涂鸦完成 2026-06-28**：`WhiteboardPage`(CustomPaint 绘制+调色板/撤销/清空)→RepaintBoundary 栅格化 PNG→走既有图片发送链路；纯 `WhiteboardController`(增点/起笔/撤销/清空)+6 单测;「+」面板「Whiteboard」入口。**实时多端共绘**属后续(可序列化 stroke 为自定义 Matrix 事件增量广播) |
| 31 | ~~圆形视频留言 Video Note~~ | ✅ **完成 2026-06-28**：相机录短视频(≤15s)→以 `n42note_` 前缀文件名(m.video 事件原生字段、收发双向天然往返,免改 mapper/元数据层)走既有视频发送链路;`message_item` 据前缀圆形渲染(ClipOval 缩略图+播放叠层);纯 `VideoNoteUtils`+2 单测;「+」面板「Video note」入口 |
| 32 | 🚧 Web 端性能优化（首批）| Flutter Web 长会话 jank。**首批 2026-06-28**:消息 `ListView.builder` 关 `addAutomaticKeepAlives`/`addSemanticIndexes`(聊天项无需滚出保活、语义索引无意义)、每条消息 Column 加 `ValueKey(message.id)` 让加载更多在顶部插历史时按 id 复用 element 非整体重建(每项 RepaintBoundary 由默认 `addRepaintBoundaries` 提供)。**真机/Web profiling 验证待在 web 环境跑**(本机 Windows 无法可靠验证 web 帧率)。后续可:图片/视频解码降采样、长列表分段加载窗口、`const` 化更多子树 |
| 33 | 无障碍深度（Semantics 全覆盖） | 🚧 **首批落地 2026-06-27**：原 chat 仓 `Semantics()` 全仓 0 命中（屏幕阅读器只播报"按钮"无描述）。本次给**最高频交互面**补语义标签：①`chat_input_bar` 语音/键盘切换、快捷回复、表情、附件、发送、按住说话按钮（`Semantics(button/label/excludeSemantics)`，复用 `commonSend/commonHoldToTalk/chatVoiceMessage` l10n + 英文兜底）；②`message_bubble` 头像播报发送者名、发送中/发送失败状态播报（"Sending"/"Failed to send, tap to resend"）。**第二批 2026-06-27**：③`conversation_tile` 整条会话合并为单个可点按语义节点（`MergeSemantics`+`ExcludeSemantics`+组合标签：名称/最后消息/时间/未读·锁·免打扰）；④`voice_message_widget` 语音条播报"Voice message, N seconds[, unplayed/playing]"；⑤`image_message_widget` 单图+九宫格补图片 alt；⑥AppBar 搜索/更多 IconButton 补 tooltip（=语义）。+4 widget 测试（`message_bubble_a11y_test`/`conversation_tile_a11y_test`）。状态描述词暂英文兜底（待补 l10n）。**第三批 2026-06-27**（按两路并行审计清单铺开 17 文件）：⑦面板/选择器——附件「+」网格(chat_more_panel)、表情反应条+快捷/完整反应选择器(message_reaction_bar)、统一表情面板 Tab(expression_panel)、emoji 退格/发送、贴纸搜索清除+商店 Tab、AI 改写关闭、GIF/成员选择器关闭、微信菜单「更多反应」；⑧消息类型——代码块复制、转账/红包/收款卡(transfer)、名片、URL 预览、视频贴纸、贴纸缩略图(sticker_thumb)、AI 链接摘要卡、message_item 的音乐/日程「加入日历」/投票项/结束投票。+2 widget 测试(`panel_reaction_a11y_test`)，全包 `dart analyze lib` 净、12 a11y 相关测试全绿。**第四批 2026-06-27**：⑨文件/位置/打赏 Tip 消息补语义标签；⑩通话消息「回拨」按钮；⑪会话列表页 AppBar 返回/新建 tooltip + 搜索栏语义。至此**聊天主链路交互面（输入栏/气泡/列表/AppBar/媒体/面板/选择器/各消息类型）已系统性覆盖**。**第五批 2026-06-27 — l10n 化**：所有无障碍状态/描述词（发送中/发送失败/未读数/语音时长/反应/贴纸/复制代码/名片/打开链接/音乐/加入日历/投票项/文件/位置/打赏/红包态等约 40 项）由新增自包含辅助 `core/utils/a11y_l10n.dart`（`A11yL10n.of(context)`）本地化，覆盖 en/简体/繁体、其余语言回退英文；不走 `S`(gen-l10n) 是因本包 gen-l10n 因历史遗留非模板 locale 重复声明 placeholder 类型而无法干净再生成（与无障碍无关）。+5 单测(`a11y_l10n_test`)。**第六批 2026-06-28 — live-region 主动播报**：`chat_page` 接入 `SemanticsService.sendAnnouncement`——对方新消息到达播报「{发送者}发来新消息：{文本}」(自己发的不报、`_lastAnnouncedIncomingId` 去重、首次进会话只记基线不报历史)、对方「正在输入」状态变化播报「{名}正在输入 / N 人正在输入」;文案经 `A11yL10n` 三语种(en/简/繁)。+test 覆盖播报文案。**第七批 2026-06-28 — 设置页表单**：审计 23 个设置页(忽略 ListTile/SwitchListTile 已具语义)，补真实缺口——`chat_background_page` 纯色/渐变色卡(GestureDetector 无标签)、`change_password`(×3)/`change_email` 密码可见性切换、`quick_replies` 编辑/删除、`security_settings` 删除 Passkey、`notification_filter` 添加 IconButton 补 tooltip；新增 `A11yL10n.solidColor/gradient/showPassword/hidePassword`(三语种)、编辑/删除/添加复用既有 `commonEdit/commonDelete/commonAdd`(gen-l10n 读路径 26 语) + 测试。**至此 #33 无障碍主链路 + 设置页表单 + live-region 均已覆盖**。**剩余长尾**：把 A11yL10n 并回 ARB（待 gen-l10n 历史类型不一致修复后）——按需再迭代 |
