...
| S2 | ~~**AI 钱包助手（只读 + 建议，第一步）**~~ 🚧 **引擎完成 2026-06-28** | `features/ai_assistant/`：`WalletAssistantEngine`（余额/持仓/Gas/帮助意图**确定性规则**回答，en+zh，8 单测）+ `WalletAiChannel` 座 + **`ChatAiChannel` 复用 n42_chat 已注册的 `AiService.completion`**（经 `GetIt.instance`，未配 key 自动降级规则回答）；chat facade 导出 `AiService`。**只读**(不签名)。剩余：UI 面板 + 从 provider 构建 `WalletSnapshot`（→ Codex 真机或后续）。设计见 `docs/钱包长期项设计稿.md` | Binance AI Wallet（雏形）|
