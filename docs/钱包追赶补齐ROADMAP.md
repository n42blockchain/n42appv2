...
| S5 | **多链长尾扩展（高价值优先）** | 16 链 | 优先补 **Base / Sui(已?) / Monad / Linea / Scroll** 等高活跃链（配置 + sender/chain_api 复用现有 blockchainType 框架）；按链逐个加 | Bitget 130 / Phantom 8+ |
...
| S8 | **交易模拟可读化补强** | tx_simulation + signature_decoder | 资产变动预览（前后余额 diff）+ 授权额度可视 + 风险高亮，向 Rabby 标杆靠 | Rabby |
...
| M2 | **稳定币消费 / Pay（Exodus Pay / Base Pay 式）** | 无 | 钱包内"扫码/一触付"稳定币支付（USDC/USDT），接商户二维码（已有收款码）+ 出入金（已有 offramp）；可先做 P2P/商户，法币结算需伙伴 | Exodus Pay / Base Pay |
