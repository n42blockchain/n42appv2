...
- 主 Flutter CI 要求仍为 70% 行覆盖率。最新完整套件通过（5,779 passed、0 failed、0 skipped），覆盖率为 **69,265 / 132,315 = 52.3486%**；现有质量门如实失败，未降低阈值、排除生成代码或跳过测试。
...
| 组件 | 本轮验证 | 新鲜覆盖率 | 门槛状态 |
|---|---|---:|---|
| Flutter 主应用 | NFT 画廊筛选/搜索/分组行为测试后完整 `flutter test --no-pub --coverage --concurrency=4 --machine`；质量门计数 5,779 通过、0 失败、0 跳过 | 69,265 / 132,315 = **52.3486%** | `.github/workflows/ci.yml` 70% 门槛保持不变，未通过 |
