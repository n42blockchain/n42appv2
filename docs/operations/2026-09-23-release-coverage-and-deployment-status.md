...
| 组件 | 本轮验证 | 新鲜覆盖率 | 门槛状态 |
|---|---|---:|---|
| Flutter 主应用 | 兼容版 Chat pin 更新、钱包 bridge、CoinPriceAlertService、token-model、Session Key、ENS 与 Paymaster 选择页 widget 回归后完整 `flutter test --no-pub --coverage --concurrency=4 --machine`；质量门计数 5,818 通过、0 失败、0 跳过 | 70,858 / 132,316 = **53.5521%** | `.github/workflows/ci.yml` 70% 门槛保持不变，未通过 |
