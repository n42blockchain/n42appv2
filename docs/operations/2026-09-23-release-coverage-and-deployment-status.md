...
- 主 Flutter CI 要求仍为 70% 行覆盖率。最新完整套件通过（5,844 passed、0 failed、0 skipped），覆盖率为 **73,118 / 132,316 = 55.2601%**；现有质量门如实失败，未降低阈值、排除生成代码或跳过测试。
...
| 组件 | 本轮验证 | 新鲜覆盖率 | 门槛状态 |
|---|---|---:|---|
| Flutter 主应用 | 兼容版 Chat pin 更新、钱包 bridge、CoinPriceAlertService、token-model、Session Key、ENS、Paymaster、AA 主页/批量交易主体、私钥导入、链详情面板、BTC 家族发送与确认、Max 费率竞态、通用转账请求金额、BrowserPage 标签页与工具栏、FIL 发送安全回归后完整 `flutter test --no-pub --coverage --concurrency=4 --machine`；质量门计数 5,844 通过、0 失败、0 跳过 | 73,118 / 132,316 = **55.2601%** | `.github/workflows/ci.yml` 70% 门槛保持不变，未通过 |
...
- 新增 FIL 发送页面的 5 项离线 widget 回归：非法/零/超余额输入及修正、接收地址命名空间前缀在 Trustdart 校验前剥离、自转和空接收地址在请求前拒绝，以及余额刷新失败提示。HttpOverrides 禁止真实网络，Trustdart/Toast 由本地 method-channel fixture 隔离；页面定向覆盖 FIL widgets 154/164 行（93.9%）、logic 61/203 行（30.0%）。完整主套件 5,844 项通过、0 失败、0 跳过，73,118/132,316 = 55.2601%；运行结果 `/tmp/n42-app-tests-fil-send.jsonl`，70% 门仍未通过。
