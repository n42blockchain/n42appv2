...
- Flutter application dependencies and actively maintained local packages: root app, `plugins/flutter_mining`, and `packages/n42_jmt_verify`.
...
- Audit the production `n42_chat` Git pin for an available compatible upstream revision. Do not edit or push the separate Chat repository. Do not treat ignored local package mirrors or `pubspec_overrides.yaml` as production sources.

## Execution phases

1. Record the clean baseline, registry resolution, tests, and known environment limitations before dependency edits.
2. Upgrade the Flutter root graph and active local Flutter packages. Migrate API changes and regenerate code only through the repository's generators.
...
- Root Flutter resolution contains 217 packages; registry metadata reported newer releases for 56 direct or development dependencies. Some current caps are documented compatibility decisions and require revalidation before removal.
