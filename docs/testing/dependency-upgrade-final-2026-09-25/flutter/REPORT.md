...
## Review fix 1 — preserve the coverage gate after Chat failure

The review found that the default GitHub Actions `success()` condition skipped the existing coverage gate when the preceding exact-source Chat suite failed. Added `id: root_tests` to the app coverage test step and `if: ${{ always() && steps.root_tests.outcome == 'success' }}` to Generate coverage report. The gate now runs whenever the root coverage test step succeeds, including when a later Chat check fails. Chat failures still fail the job; neither the Chat step nor the coverage step uses `continue-on-error`. The 70% threshold and quality-gate command are unchanged.

Verification: YAML parsed successfully; focused inspection verified the step ID, explicit condition, root → Chat → coverage order, unchanged threshold and absence of failure masking. `git diff --check` passed. No full suite was rerun because this is limited to CI scheduling. Persistent evidence: `review-fix-1.json`.
