...
## Mining example follow-up

The mining example's app script now uses Kotlin `compilerOptions.jvmTarget = JVM_21`, matching the main app and preserving its Java 21 target. A focused `flutter build apk --debug` from `plugins/flutter_mining/example` with full Homebrew OpenJDK 21.0.10 failed during configuration **before the example app script was compiled**: `plugins/flutter_mining/android/build.gradle:51` depends on `:mobile-sdk-module`, while `plugins/flutter_mining/example/android/settings.gradle.kts` includes only `:app`. The output is retained in `mining-example-build-debug.log.gz`. This is existing example bridge wiring; this scoped follow-up did not rebuild the example project. Flutter's migrator added AGP 9 compatibility flags to the example's tracked `gradle.properties` during the attempt; those incidental edits were reverted.

The supported mining verification context is the root Android project, which includes both bridge modules in `android/settings.gradle.kts` and injects Flutter embedding for the plugin. Run `cd android` followed by `JAVA_HOME=/opt/homebrew/opt/openjdk@21/libexec/openjdk.jdk/Contents/Home ./gradlew :flutter_mining:assembleDebug :flutter_mining:testDebugUnitTest --console=plain`. The full root command also ran `:app:testDebugUnitTest` and passed with that app task marked `NO-SOURCE`. The standalone plugin project does not inject Flutter embedding; its direct `assembleDebug testDebugUnitTest` attempt is recorded as failed, not as validation of the shipped plugin.

The root Gradle log warns that Gradle 8.14.4's embedded Kotlin 2.0.21 may differ from Flutter's included Gradle plugin, which requests Kotlin 2.2.20 in `/opt/homebrew/share/flutter/packages/flutter_tools/gradle/build.gradle.kts:11`. This warning originates in the fixed Flutter SDK's included build; the root APK and mining compilation completed. Java compiler warnings about obsolete source/target 8 come from upstream Android plugin scripts such as `geocoding_android` and `web3auth_flutter`, which explicitly use `VERSION_1_8`; the app and mining module remain targeted to Java/Kotlin 21. No unsupported warning was treated as a passing test or changed in external caches.
