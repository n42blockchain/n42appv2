...
`jni` 0.22 requires native calls to accept `EnvUnowned` and enter `with_env` before JNI operations. All 11 native methods were migrated. Their exported names, Java argument and return types, null/error return values, and C buffer transfer/free pattern are preserved. `with_env` catches Rust unwinding panics that cross its closure and routes them through the crate's runtime exception policy. It cannot catch a process abort caused by a panic inside an `extern "C"` function; those C ABI calls are not an unwind boundary. Null and invalid UTF-8 handling are tested, but the wrapper does not provide a general guarantee against native aborts.
