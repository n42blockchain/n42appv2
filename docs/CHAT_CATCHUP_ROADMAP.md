...
| 9 | 屏幕共享 | ⏳ **接线已完成(编译级)，待真机验证** | Dart+UI 既有；本轮补齐 Android 前台服务接线：`flutter_background` 依赖 + `livekit_service.startScreenShare` Android 分支(`Helper.requestCapturePermission()` 闸门 + `FlutterBackground` mediaProjection 前台服务，stop 时关闭) + AndroidManifest `FOREGROUND_SERVICE_MEDIA_PROJECTION` 权限与 `IsolateHolderService`(`foregroundServiceType=mediaProjection`,`tools:replace`)。Dart analyze 净、pub 解析通过；本机 APK 构建受**既有 Windows 工具链故障**(`25.0.1`,与本改动无关，stash 后同样失败)阻塞。**剩余(真机，Codex T8)**：双机验证 B 端看到 A 屏幕；iOS Broadcast Upload Extension 二阶段 |
