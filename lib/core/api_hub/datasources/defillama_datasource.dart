...
class DefiLlamaDatasource {
  ...
  static Future<List<DefiProtocol>> getProtocols({int limit = 50}) async {
    ...
    try {
      ...
      _debugLog('DefiLlamaDatasource.getProtocols error: $e');
      return (_protocolsCache ?? const []).take(limit).toList();
    }
  }
  ...
  static Future<List<DefiYield>> getYields({int limit = 50}) async {
    ...
    try {
      ...
      _debugLog('DefiLlamaDatasource.getYields error: $e');
      return (_yieldsCache ?? const []).take(limit).toList();
    }
  }

  @visibleForTesting
  static List<DefiProtocol> parseProtocolsResponse(
    dynamic raw, {
    List<DefiProtocol>? fallback,
  }) {
    if (raw == null || raw is! List) return fallback ?? const [];
    ...
    for (final item in raw) {
      ...
      if (name == null || name.isEmpty || tvl == null) continue;

      final chainList = <String>[];
      final chains = item['chains'];
      if (chains is List) {
        for (final c in chains) {
          if (c is String && c.isNotEmpty) chainList.add(c);
        }
      }
    ...
    if (protocols.isEmpty) return fallback ?? const [];
  ...
  @visibleForTesting
  static List<DefiYield> parseYieldsResponse(
    dynamic raw, {
    List<DefiYield>? fallback,
  }) {
    if (raw == null || raw is! Map) return fallback ?? const [];

    final data = raw['data'];
    if (data is! List) return fallback ?? const [];
    ...
    if (yields.isEmpty) return fallback ?? const [];
