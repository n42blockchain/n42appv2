...
class EthAPI {
  ...
  Future<MessageModel> getGasLimit(
    ...
    final gasPriceKey = get1559WithChainSymbol(coin ?? '')
        ? 'maxFeePerGas'
        : 'gasPrice';
  ...
  Future<MessageModel> getGasLimitByMap(
  ...
  /// 获取 nonce 值
  Future<MessageModel> getTransactionCount(
    String address, {
    String? coinType,
    bool isTest = false,
  }) async {
  ...
  Future<MessageModel> ethCallRaw(
    String to,
    String data, {
    String? coinType,
    bool isTest = false,
  ...
  static Future<({String name, String symbol, int decimals})?>
  getErc20TokenInfo(String contractAddress, String rpcUrl) async {
    try {
  ...
  static int _abiDecodeUint8(String hex) {
    try {
      ...
      return int.parse(
        clean.substring(clean.length > 2 ? clean.length - 2 : 0),
        radix: 16,
      );
    } catch (_) {
