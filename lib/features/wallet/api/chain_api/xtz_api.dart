...
class XtzApi {
  ...
  Future<MessageModel> getBalanceXtz(
    String address,
    String contract,
    String valueType,
    bool isTest,
    ...
    try {
      String uri = RequestUrl().getUrl2(
        CoinType.XTZ.name,
        'api',
        isTest: isTest,
      );
      var data = await BaseApi.requestEmptyH.get(
        '${uri}explorer/account/$address',
        params: {},
        defaultReturn: false,
      );
  ...
  Future<MessageModel> getCounterXtz(String address, bool isTest) async {
    try {
      ...
      var data = await BaseApi.requestEmptyH.get(
        '${uri}chains/main/blocks/head/context/contracts/$address/counter',
        params: {},
        defaultReturn: false,
      );
      MessageModel mm = MessageModel();
  ...
  Future<MessageModel> sendTxXtz(String signAddress, bool isTest) async {
    try {
      ...
      var data = await BaseApi.requestEmptyH.post(
        '${uri}injection/operation?chain=main',
        params: {},
        data: json.encode(signAddress),
        defaultReturn: false,
      );
      MessageModel mm = MessageModel();
      mm.data = data;
