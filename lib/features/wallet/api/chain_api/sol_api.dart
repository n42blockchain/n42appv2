...
class SolApi {
  ...
  Future<MessageModel> getAccountInfo(
    ...
    final result = await baseRPCSol('getAccountInfo', [
      address,
      {'encoding': 'base64'},
    ], isTest: isTest);
      ...
      if (mm.error == false) {
        ...
        if (valueMap.isNotEmpty) {
          mm.data = BigInt.parse(
            valueMap[0]['account']['data']['parsed']['info']['tokenAmount']['amount']
                .toString(),
          );
        } else {
          mm.data = BigInt.zero;
  ...
  Future<MessageModel> getFeeForMessage(
    ...
    final result = await baseRPCSol('getFeeForMessage', [
      signMessage,
    ], isTest: isTest);
    final mm = resultToMessageModel(result);
    if (mm.error == false) {
  ...
  /// 虚拟交易
  Future<MessageModel> simulateTransaction(
    String signMessage, {
    bool isTest = false,
  }) async {
    final result = await baseRPCSol('simulateTransaction', [
      signMessage,
      {'sigVerify': true},
    ], isTest: isTest);
    final mm = resultToMessageModel(result);
