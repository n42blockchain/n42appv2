...
mixin WalletConnectWidgetsMixin<T extends StatefulWidget> on State<T> {
  ...
  // ── Shared layout widgets (used by both WalletConnectPage and WalletConnectSheet) ──

  Widget buildDAppConnectWidget(WalletConnectProvider connectV2) {
    Widget connectChild = Container();
    String title = "";

    switch (connectV2.walletConnectState) {
      case WalletConnectState.loading:
        connectChild = buildPartWidget();
    ...
    return Column(
      children: [
        buildDAppInfoWidget(connectV2),
        if (title != "") buildTitleWidget(title),
        Expanded(child: connectChild),
      ],
    );
  }

  Widget buildPartWidget() {
    return Column(
      children: [
        Container(
  ...
  Widget buildDAppInfoWidget(WalletConnectProvider connectV2) {
