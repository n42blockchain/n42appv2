...
class MiningApi {
  ...
  static const String _depositContractAddress =
      '0x0dcAE65dDB5df8f1817D35286beAC32b8994962B';
  Future<Map<String, String>?> generateBls12381Keypair() async {
    try {
      ...
      if (res != null) {
        ...
        return {'privateKey': dynamicArray[0], 'publicKey': dynamicArray[1]};
      }
      return null;
    } catch (e) {
      return null;
  ...
  Future<MessageModel> getMiningWithdrawalsDaily(
    String dayStr,
    String address,
  ) async {
    try {
      ...
      var data = await BaseApi.requestEmptyH.get(
        "$_explorerApiBase/api/v2/addresses/$address/withdrawals-daily?day=$dayStr",
        params: {},
      );
      ...
      if (items != null) {
        final List<MiningWithdrawalsDaily> withdrawalsList = items
            .map(
              (item) =>
                  MiningWithdrawalsDaily.fromJson(item as Map<String, dynamic>),
            )
            .toList();
  ...
  Future<MessageModel> getMiningWithdrawalsDailySummary(String address) async {
    try {
      var data = await BaseApi.requestEmptyH.get(
        "$_explorerApiBase/api/v2/addresses/$address/withdrawals-daily/summary",
        params: {},
      );
      if (data != null) {
