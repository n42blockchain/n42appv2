...
Updated root SDK floors, four existing Flutter workflow pins, existing HIG Node pin, and seven SDK-pinned transitive lock entries. All seven are increases from e54045fe9. Included approved continuation plan amendment and corrected Task12 CallKit path as requested by controller.
...
Checks: official archive hashes match, executable versions verified, Android install exit0, git diff --check passed. No full tests rerun for declaration-only task. Root pub get after floor edits is recorded separately. Normal commit hook may increment build number.
