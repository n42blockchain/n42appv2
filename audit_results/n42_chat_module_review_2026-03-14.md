...
13. `PointsBloc` used the same `status` field for page loads, leaderboard requests, config saves, and item redemption. `RedemptionPage` therefore treated unrelated load errors as redemption failures, and a successful redeem followed by a failed balance/items refresh surfaced as a false failure even though the item was already consumed server-side. Redemption now has its own action state (`idle/inProgress/succeeded/failed`), and the page only shows snackbars for real redemption outcomes.
14. Successful redemption refreshed balance and redemption items but left recent transactions stale. The points dashboard could therefore show the new balance while still omitting the just-created redemption transaction until a manual refresh. Redemption success now refreshes transactions as well, while preserving old state if a post-success refresh call fails.
15. `PointsDashboardPage` and `LeaderboardPage` still relied on the shared `PointsState.status/errorMessage` fields for their primary loading and error UI. Opening leaderboard or loading config could therefore overwrite dashboard state, and a balance/transaction failure could surface as a fake leaderboard error. The points state now tracks independent slice statuses for balance, transactions, leaderboard, and redemption catalog, and the pages render against those specific signals instead of the global config status.
16. Room-level points config was not actually shaping the dashboard UI. Admins could disable leaderboard in config, but `PointsDashboardPage` never loaded config and still always showed the leaderboard button. It also ignored the room-level `isEnabled` flag entirely. The dashboard now loads config with the rest of its data, hides leaderboard when `showLeaderboard` is off, and shows a disabled-room notice when points are turned off.
17. `PointsConfig` omitted `pointsSymbol`, `allowTransfers`, and `showLeaderboard` from its `Equatable` props. Two configs that only differed on those fields compared equal, which undermined dirty-state detection and config-driven rebuilds. Equality now includes the full configuration surface.
18. `LeaderboardPage` and `RedemptionPage` were still directly reachable even after a room admin disabled points or turned off leaderboard in config. Hiding the dashboard shortcut alone was not enough; any direct navigation path still bypassed the room policy. Both pages now load room config on entry and render an unavailable state when the feature is disabled.
19. Governance proposal pagination depended on `state.space` even though `ProposalsListPage` never loads the space object before requesting more proposals. As a result, infinite scroll silently stopped after the first page in the normal list-only flow. `GovernanceLoadMoreProposals` now carries `spaceId`, and the bloc paginates directly against that event data instead of requiring preloaded space state.
20. Governance pagination had no dedicated “loading more” flag. The list page showed a spinner row any time `hasMoreProposals` was true, even before pagination started, and repeated bottom-scroll events could fire duplicate requests for the same next page. The bloc now tracks `isLoadingMoreProposals`, guards duplicate load-more dispatches, and the page only renders the footer spinner while a page is actually in flight.
21. `ProposalDetailPage` and `CreateProposalPage` were still coupled to the global `GovernanceStatus` stream. A stale `selectedProposal` from another detail page could briefly render under the wrong route, and unrelated governance errors could surface as snackbars on the create page before the user had submitted anything. The detail page now only renders the currently requested proposal ID and only reacts to its own load/vote lifecycle; the create page likewise only reacts to statuses after it has actually initiated a submit.
...
- `dart analyze` on `governance_event.dart`, `governance_state.dart`, `proposals_list_page.dart`, `proposal_detail_page.dart`, `create_proposal_page.dart`, and the updated/new governance page tests
- `flutter test test/unit/blocs/governance_bloc_test.dart test/presentation/pages/governance/proposals_list_page_test.dart test/presentation/pages/governance/proposal_detail_page_test.dart test/presentation/pages/governance/create_proposal_page_test.dart`
- `dart analyze` on `points_bloc.dart`, `points_state.dart`, `redemption_page.dart`, and the new points bloc/redemption page tests
- `flutter test test/unit/blocs/points_bloc_test.dart test/presentation/pages/redemption_page_test.dart`
- `dart analyze` on `points_dashboard_page.dart`, `leaderboard_page.dart`, `points_config.dart`, and the new dashboard/leaderboard page tests
- `flutter test test/presentation/pages/points_dashboard_page_test.dart test/presentation/pages/leaderboard_page_test.dart`
- `dart analyze` on `redemption_page.dart` and the updated redemption-page tests
- `flutter test test/presentation/pages/redemption_page_test.dart`

Residual risk:

- `presentation/pages/points` is now largely covered for current UI flows, but leaderboard pagination and the broader proposal/points overlap remain open.
...
12. `VoiceRoomService.joinRoom(...)` had no guard for rejoining the same room or switching between rooms while already connected. Because the list page and room page share the same singleton service, create-and-open flows could hit duplicate backend `joinVoiceRoom(...)` calls for the same room, and cross-room navigation could join the next room without first leaving the previous one. The service now no-ops same-room rejoins and explicitly leaves the prior room before joining a different one.
13. `VoiceRoomPage._confirmEndRoom(...)` still popped the room page immediately after dispatching `EndVoiceRoom()`. That bypassed the later disconnect-driven exit logic, so a failed end-room request still kicked the host out of the screen and hid the failure. The dialog now closes only itself; page exit remains tied to `isConnected -> false`, and failed end-room requests surface an error instead of silently leaving.
14. `MiniAppPage` allowed any HTTPS top-level navigation, while `MiniAppBridgeService` registered wallet/chat JS channels globally and injected `window.n42` on every page finish. A trusted mini app could therefore redirect to an unrelated HTTPS origin and still retain bridge access. The mini-app container now enforces same-origin navigation against the app manifest URL, rejects invalid/non-HTTPS launch URLs up front, and the bridge itself double-checks the current page origin before honoring wallet/chat requests.
15. Wallet bridge requests had no deterministic failure response when the page origin became untrusted, the page was torn down mid-request, or the mini app sent an unknown wallet method. That left JavaScript promises hanging forever. The bridge now resolves those calls with explicit error callbacks instead of silently logging and dropping them.
16. `VoiceRoomListPage` still closed the create-room dialog immediately after dispatching `CreateVoiceRoom(...)`. If creation or auto-join failed, the dialog was already gone and the typed room name/topic were lost; on success, dialog dismissal and route navigation raced each other. The dialog is now scoped to the same bloc, keeps local input on failure, shows inline errors, and only closes plus navigates after confirmed create-and-join success.
17. `SocialGraphRepositoryImpl.getProfile(...)` treated DeBank portfolio, chain lookup, and reverse-identity lookup as an all-or-nothing `Future.wait(...)`. If any single upstream timed out or errored, the entire profile load failed even though partial data was still available. The repository now degrades each upstream independently and returns the richest partial profile it can assemble.
18. `SocialGraphRepositoryImpl.searchProfiles(...)` also failed closed on resolver exceptions. A temporary ENS resolver failure could wipe out otherwise valid Lens/Farcaster matches for the same query. Resolver failures are now isolated per provider so surviving identity hits still return results.
19. `AiRepositoryImpl` persisted assistants and chat history with unsynchronized read-modify-write cycles. Concurrent `saveAssistant(...)` or `saveChatMessage(...)` calls could overwrite each other and silently drop one update. Assistant writes and per-assistant history writes are now serialized so concurrent saves no longer lose data.
20. `SocialGraphService.getRecommendations(...)` removed the querying address from candidates using `holderSet.remove(address.toLowerCase())`, but collection-owner payloads were never normalized. If Alchemy returned the current address in checksum or uppercase form, the service could still recommend the user to themselves. Candidate collection is now normalized before dedupe and self-filtering, so self recommendations are filtered regardless of case.
21. `SocialGraphService.getRecommendations(...)` truncated candidates with `holderSet.take(maxCandidates)` before any scoring, which made the final recommendation set depend on owner enumeration order instead of actual overlap strength. A stronger candidate discovered later could be dropped entirely while weaker earlier owners were scored. Candidate selection now prioritizes addresses that appear across more of the user’s shared NFT collections before applying the candidate cap.
22. `AiAssistantBloc` accepted `AiStreamChunkReceived/AiStreamCompleted/AiStreamError` events without tying them to the currently active request. After switching assistants, clearing history, or stopping generation, late events from the old stream could still append text or save an assistant reply into the new session. Stream events now carry a request ID, and the bloc ignores stale generations after cancellation or assistant switches.
23. `AiDatasource.streamCompletion(...)` decoded each SSE byte chunk independently and only processed buffered data when a newline arrived. That made the parser vulnerable to UTF-8 multi-byte characters split across chunks and to a final SSE line that ended without a trailing newline. The stream parser now uses a proper UTF-8 decoder pipeline with `LineSplitter`, so split code points and trailing non-newline events are handled correctly.
24. `SocialGraphService.getRecommendations(...)` still assumed the initial NFT-discovery call would succeed. If Alchemy failed before any candidate collection began, the entire recommendation flow threw instead of degrading to an empty result the UI could handle. Initial NFT discovery now fails soft and returns no recommendations rather than aborting the feature.
25. `SocialGraphService._tokenOverlapScore(...)` compared token lists on the first chain returned for address A, even when that chain was not shared with address B. That made token overlap depend on chain ordering and systematically under-scored wallets whose real overlap lived on a later shared chain. Token overlap now aggregates symbols across the first few common chains between both addresses and returns zero cleanly when they share none.
26. `AiDatasource.completion(...)` and `streamCompletion(...)` only accepted assistant content as a plain string. Several OpenAI-compatible backends return content parts arrays instead (`[{type: "text", text: "..."}]`), which previously parsed as an empty reply. The datasource now extracts text from both plain-string and content-part payloads for normal and streaming completions.
27. `AiDatasource.streamCompletion(...)` treated inline SSE error payloads as ignorable malformed chunks. Providers that emit `data: {"error": ...}` mid-stream would therefore hang or end silently instead of surfacing a usable failure. Stream error events are now converted into `AiServiceException` so callers get a deterministic failure path.
28. `AiAssistantPage` kept the send button live even when the draft was empty or the AI service was unavailable, and `_sendMessage()` cleared the local draft before rechecking whether the bloc would actually accept the request. The page now derives a real `canSend` state from availability, generation status, and draft text, disables the send affordance accordingly, and only clears the draft after a valid dispatch.
29. `AiAssistantBloc._onSendMessage(...)` awaited `saveChatMessage(...)` before stream setup without guarding persistence failures. A local storage error could therefore interrupt the whole chat send path and leave the UI stuck in generating state even though the AI service itself was healthy. Message persistence is now best-effort for user/assistant messages, while clear-history and switch-assistant failures also surface explicit errors instead of throwing out of the bloc handler.
30. `SocialGraphService.getRecommendations(...)` fetched NFT owner lists strictly serially. A single slow `getCollectionOwners(...)` call delayed discovery for every later collection, even though the collection fan-out is already capped to five requests. Owner discovery now runs in parallel across the shortlisted collections while still keeping the overall collection cap.
31. Candidate recommendation scoring also ran strictly serially. With up to ten shortlisted addresses, one slow candidate could block all later candidates and stretch recommendation latency toward the sum of every upstream timeout. Candidate scoring is now processed with bounded concurrency, so later candidates can progress while an earlier one is slow without exploding total request fan-out.
32. Recommendation scoring repeatedly re-fetched the querying address’s chains, NFTs, and source-chain token lists for every candidate. That amplified the same upstream calls N times within one recommendation run and wasted time on the hottest path. The recommendation path now reuses source-side chains/NFTs and caches source token lists by chain across candidates. This also fixed case-sensitive NFT collection comparison, so checksum/casing differences no longer erase real shared collections.
33. `SocialGraphBloc` exposed one shared `status/errorMessage` for profile loading, recommendations, similarity calculation, and search. Because `SocialGraphPage` fires profile and recommendation loads together, any one request could flip the entire feature into a global error or loading state even while another dataset had already loaded. The bloc now tracks per-operation status and error fields instead of a single shared lifecycle.
34. `SocialGraphPage` and `UserSimilarityPage` both consumed that shared bloc lifecycle directly. A recommendation failure could hide an already loaded profile, and unrelated recommendation/search failures could bleed into the similarity detail page. The pages now read the operation-specific state they actually depend on, so one subrequest no longer blanks unrelated content.
35. `SocialGraphPage` still routed recommendation taps into `UserSimilarityPageStub`, a placeholder scaffold that never showed real similarity data. Recommendation cards now navigate to the actual `UserSimilarityPage` with the shared bloc and target addresses, and widget coverage locks that path so it cannot silently regress back to a stub.
36. `UserSimilarityPage` still rendered synthetic breakdown bars and placeholder “common items” even after navigation was fixed. The bloc only stored a scalar `similarityScore`, so the page had no access to the real shared tokens, NFT collections, or chains that `SocialGraphService.calculateSimilarity(...)` already computed. The similarity pipeline now carries the full `SocialSimilarityModel` through the repository and bloc state, and the page renders real overlap dimensions and shared-item lists instead of fabricated percentages and labels.
...
- `flutter test test/unit/services/voice_room_service_test.dart`
- `flutter test test/unit/blocs/voice_room_bloc_extended_test.dart test/presentation/pages/voice_room_page_test.dart`
...
- `dart analyze` on `mini_app_bridge_service.dart`, `mini_app_page.dart`, `voice_room_list_page.dart`, and the new page/service tests
- `flutter test test/unit/services/mini_app_bridge_service_test.dart test/presentation/pages/mini_app_page_test.dart`
- `flutter test test/presentation/pages/voice_room_list_page_test.dart`
- `dart analyze` on `ai_repository_impl.dart`, `social_graph_repository_impl.dart`, and the updated repository tests
- `flutter test test/unit/repositories/ai_repository_impl_test.dart test/unit/repositories/social_graph_repository_impl_test.dart`
- `dart analyze` on `social_graph_service.dart` and the new service tests
- `flutter test test/unit/services/social_graph_service_test.dart test/unit/repositories/social_graph_repository_impl_test.dart`
- `flutter test test/unit/services/social_graph_service_test.dart`
- `dart analyze` on `ai_datasource.dart`, `ai_assistant_bloc.dart`, `ai_assistant_event.dart`, and the updated datasource/bloc tests
- `flutter test test/unit/datasources/ai_datasource_test.dart test/unit/blocs/ai_assistant_bloc_test.dart test/unit/blocs/ai_assistant_bloc_extended_test.dart`
- `dart analyze` on `ai_assistant_page.dart`, `ai_assistant_bloc.dart`, and the new AI page test
- `flutter test test/unit/blocs/ai_assistant_bloc_extended_test.dart test/presentation/pages/ai_assistant_page_test.dart`
- `dart analyze` on `social_graph_service.dart` and the expanded service tests
- `flutter test test/unit/services/social_graph_service_test.dart`
- `dart analyze` on `social_graph_bloc.dart`, `social_graph_state.dart`, `social_graph_page.dart`, `user_similarity_page.dart`, and the new bloc test
- `flutter test test/unit/blocs/social_graph_bloc_test.dart`
- `dart analyze` on `social_graph_page_test.dart` and `user_similarity_page_test.dart`
- `flutter test test/presentation/pages/social_graph_page_test.dart test/presentation/pages/user_similarity_page_test.dart`
- `dart analyze` on `social_graph_repository.dart`, `social_graph_repository_impl.dart`, `social_similarity_model.dart`, and the updated similarity page/repository tests
- `flutter test test/unit/blocs/social_graph_bloc_test.dart test/presentation/pages/user_similarity_page_test.dart test/unit/repositories/social_graph_repository_impl_test.dart`
...
- `social_graph_repository_impl.dart` still returns minimal `SocialProfile` objects with sparse metadata when enrichment providers fail. Availability is better, but provenance/quality scoring for partial profiles is still absent.
- `social_graph_service.dart` no longer scores candidates strictly serially, but it still issues candidate similarity work as independent network-heavy tasks without cross-request memoization beyond the current source-address cache. Repeated recommendation refreshes can still re-hit the same downstream addresses.
- `AiDatasource` still trusts upstream SSE payload shape beyond the `choices[0].delta.content` path and skips malformed lines silently. Availability is improved, but per-provider protocol drift still has limited observability beyond debug logs.
