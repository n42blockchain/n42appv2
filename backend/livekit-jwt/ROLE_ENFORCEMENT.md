...
## 2. 判据：谁是主播，由 homeserver 说了算

不看客户端说自己是谁，改为服务端自己查 Matrix 房间状态：
...
| 请求带的 role | 服务端查到的创建者 | 结果 |
|---|---|---|
| `broadcaster` | 等于请求者 | ✅ 可推流 |
| `broadcaster` | 不等于请求者（伪造） | ❌ 只能观看 |
| `viewer` | 任意 | ❌ 只能观看 |
| 其它未知值 | 任意 | ❌ 只能观看（默认拒绝） |
| **不带 role** | 任意 | ✅ 可推流（见下） |
| `broadcaster` | 查不到创建者 | ❌ 只能观看（失败关闭） |

**不带 role 的请求保持可推流**：那是聊天的群通话，每个参与者都要发言，
这条路径的行为不能被本次改动影响。直播客户端一定会带 role。
...
3. **伪造 role 挡得住**：手工发一个请求，`identity` 用 B、`role` 填
   `broadcaster`、`conversation_id` 填 A 的房间：
